...
from constants.constants import Fs, Fc, SinOrCosType, LowerFrequency, UpperFrequency, DefaultAlignFramesNum, \
     Amplitude, LowPassFrequency, DCIRLowPassFrequency, DeviceType
...
class AudioUtils(object):
    ...
    @classmethod
    def d_cir_low_pass(cls, signals, lower_fre=DCIRLowPassFrequency):
        [b, a] = signal.butter(8, lower_fre * 2 / Fs, btype='lowpass', output='ba')
        return signal.filtfilt(b, a, signals)
    ...
    @classmethod
    def write_audio(cls, audio_signal, file_path, data_type=np.int16):
        """
        write the signals into a wav file at file_path
        ...
        =====================  ===========  ===========  =============
        :param data_type: int16 or float32
        :param audio_signal: the signal
        :param file_path: the target path
        :return: None
        """
        if data_type == np.int16:
            audio_signal = audio_signal * Amplitude
            write(file_path, Fs, audio_signal.astype(np.int16))
        else:
            write(file_path, Fs, audio_signal.astype(np.float32))
...
if __name__ == '__main__':
    data = AudioUtils.read_audio(r"C:\Users\zengq\Desktop\acouInput_matlab\barker-time.wav")
    show_signals(data[:, 0])
    show_fft(data[:, 0])
